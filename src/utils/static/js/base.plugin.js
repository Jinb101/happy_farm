...
var demo_es6 = function (cls) {
  ...
  let imageUrlToFile = (url) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        blob = xhr.response;
        let img = new File([blob], "pic_" + ts.rand(6), {
          type: "image/" + type,
        });
        resolve(img);
      };
        ...
        if (compress) {
          imageUrlToFile(src)
            .then((res) => {
              imageCompress(
                res,
                { with: img.width, height: img.height, type: "file" },
                (ret) => {
                  o.data = ret;
                  resolve(o);
                }
...
var demo_vx = function (cls) {
  ...
  let geo = function (fn, type = "gcj02") {
    ...
    wx.getLocation({
      type, // 默认为 wgs84 的 gps 坐标，如果要返回直接给 openLocation 用的火星坐标，可传入'gcj02'
      success: function (res) {
  ...
  let geoOpen = function (obj, fn) {
    ...
    wx.openLocation({
      ...
      scale: obj.scale || 15, // 地图缩放级别，整形值，范围从 1~28。默认为最大
...
class DemoAMap {
  ...
  circle(opt, fn) {
    ...
    if (opt.lng && opt.lat) {
      if (opt.type === "polygon") {
        ...
        opt.lng.forEach((r, v) => {
          this._circle_handler(
            { lng: r, lat: opt.lat[v], type: opt.type, pointer: 1 },
            (e) => {
              fn ? fn(e) : null;
            }
          );
  ...
  circularOption(opt) {
    ...
    if (con.radius) {
      os.radius = con.radius;
    }
    if (con.center && con.center.length === 2) {
      os.center = con.center;
    }
    if (con.lng && con.lat) {
      os.center = [con.lng, con.lat];
    }
    ...
    AMap.plugin("AMap.CitySearch", function () {
      ...
      citySearch.getLocalCity(function (status, result) {
        if (status === "complete" && result.info === "OK") {
          ...
          fn
            ? fn(
                ts.code(status + ":" + JSON.stringify(result), 0, {
                  status,
                  result,
                })
              )
        ...
        if (status === "complete" && result.info === "OK") {
          ...
          let os = {
            ...
            list: (c.pois || [])
              .filter((r) => {
                return (
                  r.name && r.address && ts.demo._is_obj(r.address).code === 3
                );
              })
              .map((v) => {
